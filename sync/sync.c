...
/* Makes any changes to a routing table or mac list based on the instructions
 * encoded in sync_msg. */
void process_sync_mesg(dll_t *dll, sync_msg_t *sync_msg) {
  dll_node_t *node;
  if (sync_msg->l_code == L3) {
    node = find_routing_table_entry(
        dll, sync_msg->msg_body.routing_table_entry.dest,
    ...
    case UPDATE:
      if (node != dll->head) {
        update(node, sync_msg->msg_body.routing_table_entry.gw,
    ...
    case DELETE:
      if (node != dll->head) {
        ...
        if (node == dll->head) {
          ...
          unlink(sync_msg->msg_body.mac_list_entry
                     .mac); // deallocate shared memory region corresponding to
                            // this MAC key
        }
      }
      break;
